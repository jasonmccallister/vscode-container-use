...
export class ContainerUseCli {
    ...
    /**
     * Watch for changes with streaming output
     * Returns a child process that can be used to stream output in real-time
     */
    startWatchStream(): { process: ReturnType<typeof spawn>; promise: Promise<CliResult> } {
        console.log('Starting cu watch stream...');
        
        const childProcess = spawn('cu', ['watch'], {
            stdio: ['ignore', 'pipe', 'pipe'],
            cwd: this.workspacePath,
            env: {
                ...process.env,
                // Disable TTY-related features that cause issues in non-interactive mode
                TERM: 'dumb',
                // Disable color output which can cause TTY issues
                NO_COLOR: '1',
                FORCE_COLOR: '0',
